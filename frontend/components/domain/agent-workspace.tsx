...
export function AgentWorkspace({
  ...
  // Images selected but not yet sent to the agent — held locally until the
  // user submits a message, at which point they are flushed to the parent so
  // the upload begins and the post preview is updated.
  ...
  function submitInstruction(text: string) {
    ...
    // Allow submitting with no text when there are pending images — the user
    // is confirming the attachment without adding an instruction.
    if (!nextText && pendingImages.length === 0) {
      return
    }

    // Flush any pending images to the parent now that the user is sending a
    // message. This triggers the upload in EditorLayout and adds them to the
    // post preview.
  ...
  function handleImageUpload(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    // Stage locally — do NOT pass to parent yet. Images are only flushed to
    // the parent (and therefore uploaded + shown in the post preview) when the
  ...
  function removeImage(id: string) {
    // Pending images are local-only; revoke their object URL and drop them.
    const pending = pendingImages.find((i) => i.id === id)
    if (pending) {
      URL.revokeObjectURL(pending.url)
      setPendingImages((prev) => prev.filter((i) => i.id !== id))
      return
    }
    // Confirmed (already-uploaded) images are owned by the parent.
    ...
    <section className="flex h-full min-h-0 flex-col overflow-hidden rounded-[36px] bg-[radial-gradient(circle_at_top_left,rgba(219,234,254,0.85),transparent_38%),linear-gradient(180deg,#ffffff_0%,#f8fafc_100%)] p-5 shadow-[0_24px_80px_rgba(15,23,42,0.08)] ring-1 ring-slate-200/80 dark:bg-[radial-gradient(circle_at_top_left,rgba(30,64,175,0.24),transparent_38%),linear-gradient(180deg,#0f172a_0%,#020617_100%)] dark:shadow-[0_24px_80px_rgba(2,6,23,0.5)] dark:ring-slate-800">
      ...
      <div className="mt-5 border-t border-slate-200/80 pt-4 dark:border-slate-800">
        {(images.length > 0 || pendingImages.length > 0) && (
          <div className="mb-3 flex flex-wrap gap-2">
            ...
            {pendingImages.map((img) => (
              <div key={img.id} className="group relative h-16 w-16 overflow-hidden rounded-xl border-2 border-dashed border-amber-400 dark:border-amber-500" title="Send a message to attach this image">
                <img src={img.url} alt={img.file?.name ?? 'pending image'} className="h-full w-full object-cover opacity-70" />
                <div className="pointer-events-none absolute bottom-0 left-0 right-0 bg-amber-500/80 py-0.5 text-center text-[9px] font-semibold uppercase tracking-wide text-white">
                  Pending
                </div>
                <button
                  type="button"
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            type="button"
            onClick={() => submitInstruction(input)}
            disabled={isPending || (input.trim() === '' && pendingImages.length === 0)}
            className="flex h-14 w-14 shrink-0 items-center justify-center rounded-[22px] border border-slate-200 bg-white/90 text-slate-500 transition hover:border-blue-300 hover:text-blue-600 disabled:opacity-40 dark:border-slate-700 dark:bg-slate-900/90 dark:text-slate-400 dark:hover:border-blue-700 dark:hover:text-blue-400"
          >
            <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
